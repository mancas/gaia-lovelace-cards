...
export class ButtonCard extends LitElement {
  ...
  render() {
    ...
    const name =
      this._config.name ?? (entity?.attributes?.['friendly_name'] as string) ?? this._config.entity;
    const icon = this._config.icon ?? this._defaultIcon();

    const stateLabel = isFan && isOn && fanPct ? `on • ${fanPct}%` : (entity?.state ?? '');
    ...
    return html`
      ...
      >
        ...
        ${
          isFan && isOn
            ? html` <div class="fan-speed" @click=${(e: Event) => e.stopPropagation()}>
