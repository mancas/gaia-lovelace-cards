...
defineEditor(
  'custom-power-monitor-card-editor',
  [
    {
      name: 'entity',
      required: true,
  ...
  {
    labels: { entity: 'Power sensor' },
    helpers: { unit: "Overrides the sensor's own unit" },
  },
);
...
export class PowerMonitorCard extends LitElement {
  ...
  static styles = css`
    ...
    .power-value.low {
      color: var(--custom-power-low-color, #4caf50);
    }
    .power-value.medium {
      color: var(--custom-power-medium-color, #ff9800);
    }
    .power-value.high {
      color: var(--custom-power-high-color, #f44336);
    }
    .power-unit {
      font-size: 1rem;
      font-weight: 400;
  ...
  render() {
    ...
    const displayVal = isNaN(watts)
      ? '—'
      : watts >= 1000
        ? `${(watts / 1000).toFixed(2)}`
        : `${watts.toFixed(1)}`;
    const displayUnit = isNaN(watts)
      ? this._config.unit
      : watts >= 1000
        ? 'kW'
        : (this._config.unit ?? 'W');
