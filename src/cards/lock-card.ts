...
export class LockCard extends LitElement {
  ...
  static styles = css`
    ...
    ha-card.unlocked {
      border-color: var(--custom-lock-unlocked-color, var(--warning-color, #ff9800));
    }
    ha-card.jammed {
      border-color: var(--custom-lock-jammed-color, var(--error-color, #f44336));
    }
    ha-card.unavailable {
      opacity: 0.5;
      pointer-events: none;
    ...
    .lock-icon.locked {
      color: var(--custom-lock-locked-color, var(--primary-color, #03a9f4));
    }
    .lock-icon.unlocked {
      color: var(--custom-lock-unlocked-color, var(--warning-color, #ff9800));
    }
    .lock-icon.jammed {
      color: var(--custom-lock-jammed-color, var(--error-color, #f44336));
    }
    .name {
      font-size: 0.9rem;
      font-weight: 500;
  ...
  private _handleAction() {
    ...
    if (entityState === 'unlocked') {
      ...
      const name =
        this._config.name ??
        (this._entity?.attributes?.['friendly_name'] as string) ??
        this._config.entity;
