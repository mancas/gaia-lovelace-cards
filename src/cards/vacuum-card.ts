...
defineEditor('custom-vacuum-card-editor', [
    ...
    return html`
      <ha-card>
        <div class="icon-wrapper">
          ...
          ${
            battery != null
              ? html`<span class="battery-badge"
                  ><ha-icon icon="mdi:battery" style="--mdc-icon-size:10px"></ha-icon
                  >${battery}%</span
                >`
              : nothing
          }
        ...
        <div class="controls">
          ...
          <ha-icon-button .label=${'Return to base'} @click=${() => this._call('return_to_base')}
            ><ha-icon icon="mdi:home-map-marker"></ha-icon
          ></ha-icon-button>

          <ha-icon-button .label=${'Locate'} @click=${() => this._call('locate')}
